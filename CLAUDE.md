...
# Training with text encoder on the random-goal environment
...
**Goal-grid environment** (`cookie_env.envs.goal_grid.GoalGrid`, from the sibling **cookie-env** package): a minigrid-style room where the agent must reach a green square. One class covers both variants via `goal_pos` — a tuple pins the square (`env.goal_pos_x/y`), `null` on either axis resamples it every episode. This replaces the former `envs/fixed_goal.py` / `envs/random_goal.py` pair, which was the same env duplicated. The `env=fixed_goal` / `env=random_goal` presets and the `fixed-goal_` / `random-goal_` task strings still work — archived runs carry them in `.hydra/config.yaml` — and both now build the same class. Score -1001 is the floor (goal never reached within the 1000-step time limit); learning shows as scores rising above ~-500.
