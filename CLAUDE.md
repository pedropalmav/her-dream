...
Diagnostic scripts live in `experiments/` (WM/text-encoder stochasticity, posterior consistency across trajectories, `text_wm_alignment.py` which pairs missions with WM posteriors over real rollouts). `success_rate.py` is the comparable benchmark: a frozen suite of 1000 `(initial_state, goal_state)` pairs (`experiments/common/pairs.py`) run against any number of checkpoints, reporting how often the policy reaches the goal pose next to a random-action floor and the BFS-oracle z-check. One suite per env, picked by `--suite` (default: from the run's task) — `random_goal` samples the tasks (random spawn pose, goal cell drawn independently of the green square, ≥4 BFS actions away), while `fixed_goal` *enumerates* them: from the one spawn the agent trained at, every interior cell in every facing (the 252-pose `sweep` block), plus 748 sampled pairs, reported per block since only the sweep is in-distribution. Results land in `experiments/out/success_rate_<suite>.{json,png}`, plus a per-cell success map for the sweep. Shared helpers live in `experiments/common/` (`grid.py` BFS/geometry, `pairs.py` suite, `oracle.py`, plus loading/rollout/metrics/io) — put anything reusable there rather than in a script. `tools/`/`viz/` contain the interactive Crafter dash and trajectory replay utilities.
