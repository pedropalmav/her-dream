...
## Goal-conditioned rewards (`rewards.py`)
...
- `imagination` — at episode start, roll out the (frozen) WM for `imag_horizon` (15) steps with uniformly random actions from the first observation and take the final prior `z` as the goal, so it is reachable from the current episode by construction (`Dreamer.imagine_goal`).
- `image` — at episode start, render the observation of a synthetic state with an auxiliary `FixedGoal` env (`envs.fixed_goal.GoalImageGenerator`): green square at the current episode's goal position, agent at a random cell. The frozen WM encodes it into `z` with one posterior step from the initial state (`Dreamer.observe_goal`). Designed for `random_goal`, where buffer-sampled goals encode the wrong square position.

`GoalConditioned` (`envs/wrappers.py`) adds the `goal` key to observations: `(K,)` one-hot for `first_row`, `(S, K)` for everything else. Dimensions are enforced via `env.stochastic_classes = model.rssm.discrete` and `env.stochastic_rows = model.rssm.stoch` in the config.
...
**Configs**: Hydra composes from `configs/configs.yaml` (top-level), `configs/env/<env>.yaml`, `configs/model/<size>.yaml`, and `configs/buffer/<type>.yaml` (`her` with her_ratio=0.8/strategy=final, or `normal`). The default env is `random_goal` and default model is `size12M`. All model hyperparameters live in `configs/model/_base_.yaml`. Every run saves its composed config to `<logdir>/.hydra/config.yaml` — always check that, not the current defaults, when analyzing an old run.
