...
- `imagination` — at episode start, roll out the (frozen) WM + actor for `imag_horizon` (15) steps from the first observation and take the final prior `z` as the goal, so it is reachable from the current episode by construction (`Dreamer.imagine_goal`).
