...
`clone_and_freeze`, `ActorCritic.update_slow_target`, and `train()` keeping the
slow value network in eval mode.
"""
...
class TestApplyFreezeWM:
    ...
    def test_actor_value_still_trainable(self):
        agent, _ = make_real_dreamer(freeze_wm=True)
        assert _any_trainable(agent.ac.actor)
        assert _any_trainable(agent.ac.value)
...
class TestApplyTrainTextOnly:
    def test_freezes_everything_but_text_encoder(self):
        ...
        assert _all_frozen(agent.encoder)
        assert _all_frozen(agent.rssm)
        assert _all_frozen(agent.ac.actor)
        assert _all_frozen(agent.ac.value)
...
class TestCloneAndFreeze:
    ...
    def test_world_model_frozen_networks_present(self, default_dreamer):
        for name in ("_frozen_encoder", "_frozen_rssm"):
            assert hasattr(default_dreamer, name)

    def test_actor_critic_frozen_networks_present(self, default_dreamer):
        # The policy-side clones belong to the ActorCritic sub-module.
        for name in ("_frozen_actor", "_frozen_value", "_frozen_slow_value"):
            assert hasattr(default_dreamer.ac, name)


class TestSlowTarget:
    def test_slow_value_frozen(self, default_dreamer):
        assert _all_frozen(default_dreamer.ac._slow_value)
    ...
    def test_update_slow_target_mixes_on_schedule(self, default_dreamer):
        ...
        with torch.no_grad():
            for p in agent.ac.value.parameters():
                p.add_(1.0)
        before = [s.data.clone() for s in agent.ac._slow_value.parameters()]
        agent.ac.update_slow_target()  # _slow_value_updates starts at 0 -> mixes
        moved = any(not torch.equal(b, s.data) for b, s in zip(before, agent.ac._slow_value.parameters()))
        assert moved
    ...
    def test_update_slow_target_skips_off_schedule(self):
        ...
        agent.ac._slow_value_updates = 1
        with torch.no_grad():
            for p in agent.ac.value.parameters():
                p.add_(1.0)
        before = [s.data.clone() for s in agent.ac._slow_value.parameters()]
        agent.ac.update_slow_target()
        unchanged = all(torch.equal(b, s.data) for b, s in zip(before, agent.ac._slow_value.parameters()))
        assert unchanged
...
class TestTrainMode:
    def test_slow_value_stays_eval_after_train(self, default_dreamer):
        ...
        assert default_dreamer.ac._slow_value.training is False
