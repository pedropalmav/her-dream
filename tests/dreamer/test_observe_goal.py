"""Tests for `Dreamer.observe_goal` (goal_sample="image").

`observe_goal` encodes a rendered goal observation into a goal z with a SINGLE
...
class TestOutputShape:
    def test_full_returns_N_S_K(self, make_agent, goal_image_obs):
        goal = make_agent(goal_type="full").observe_goal(goal_image_obs)
        assert goal.shape == (N, S, K)

    def test_argmax_full_returns_N_S_K(self, make_agent, goal_image_obs):
        goal = make_agent(goal_type="argmax_full").observe_goal(goal_image_obs)
        assert goal.shape == (N, S, K)

    @pytest.mark.parametrize("n", [1, 5, 8])
    def test_batch_dim_follows_obs(self, make_agent, n):
        goal = make_agent(goal_type="full").observe_goal(make_goal_image_obs(n))
        assert goal.shape == (n, S, K)
...
class TestGoalIsOneHot:
    ...
    def test_goal_is_one_hot(self, make_agent, goal_image_obs, goal_type):
        goal = make_agent(goal_type=goal_type).observe_goal(goal_image_obs)
...
class TestSinglePosteriorStep:
    ...
    def test_obs_step_called_once(self, make_agent, goal_image_obs, rssm, monkeypatch):
        ...
        make_agent(goal_type="full").observe_goal(goal_image_obs)
        assert spy.count == 1
...
class TestNoGrad:
    ...
    def test_goal_does_not_require_grad(self, make_agent, goal_image_obs, goal_type):
        # observe_goal is @torch.no_grad: goals are detached targets, not learnable.
        goal = make_agent(goal_type=goal_type).observe_goal(goal_image_obs)
