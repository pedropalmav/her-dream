...
def make_actions(exp_type: str, n_spin: int, n_forward: int) -> list[int]:
    ...
    if exp_type == "spin_left":
        return spin(k, "left")
    if exp_type == "spin_right":
        return spin(k, "right")
    if exp_type == "loop_right":
        return loop(side, "right")
    if exp_type == "loop_left":
        return loop(side, "left")
    if exp_type == "loop_right_and_spin":
        return loop(side, "right") + spin(k)
    if exp_type == "loop_left_and_spin":
...
def create_app(agent, env_factory, device: str, seed: int) -> Dash:
                                    ...
                                    *[
                                        html.Div(
                                            [
                                                ...
                                                html.Span(ACT_NAMES[k], style={"fontSize": "11px", "color": "#444"}),
                                            ],
    ...
    def _run(n_clicks, exp_type, n_spin, n_forward):
        ...
        print(f"[run] exp={exp_type}  n_spin={n_spin}  n_fwd={n_forward}  len(actions)={len(actions)}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Dash app para visualizar trayectorias del World Model.")
    ...
    parser.add_argument("--seed", type=int, default=0, help="Seed para el reset del env en cada ejecución")
    parser.add_argument("--port", type=int, default=8050, help="Puerto del servidor Dash")
